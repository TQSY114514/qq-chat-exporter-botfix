    ...
    private setupRoutes(): void {
        ...
        this.app.get('/api/recent-contacts', async (req, res) => {
            try {
                ...
                // Issue #364: getRecentContactListSnapShot 只返回最近活跃的会话快照，
                // count 也有限。很多 QQ 机器人 / 服务号 / 临时会话若最近没有消息往来
                // 就不在快照里，导致前端列表看不到，无法选中导出。
                // 这里在 includeAll=true 时额外调用无 count 参数的「全量本地会话」接口，
                // 把快照窗口之外、但本机 NTQQ DB 仍存在的会话捞回来，按 chatType|peerUid 去重。
                if (includeAll) {
                    const seen = new Set(
                        changedList.map(
                            (c: any) => `${c?.chatType}|${c?.peerUid}`,
                        ),
                    );
                    for (const methodName of [
                        ...
                        const fn = userApi?.[methodName];
                        if (typeof fn !== 'function') continue;
                        try {
                            const extra = await fn.call(userApi);
                            // 返回形状假定与 SnapShot 一致（{ info: { changedList: [] } }），
                            // 兼容裸数组形态。
                            const extraList: any[] =
                                extra?.info?.changedList ||
                                (Array.isArray(extra) ? extra : []);
                            for (const c of extraList) {
                                if (!c?.peerUid || c?.chatType === undefined || c?.chatType === null) {
                                    continue;
                                }
                                const key = `${c.chatType}|${c.peerUid}`;
                            ...
                            this.safeLogger.logWarn(
                                `[ApiServer] /api/recent-contacts: ${methodName} 失败，跳过`,
                                e,
                            );
                        }
                    }
                ...
                const contacts = changedList
                    ...
                    .map(c => {
                        ...
                        // Issue #364: 很多 QQ 机器人 / 服务号在 NTQQ 内部 chatType 就是 1（普通
                        // C2C 私聊），只是对方不在好友列表。这种情况原本会被归为 'private'，而前端
                        // 只合并 'special'，导致这类机器人会话在列表里看不到。includeAll=true 时把
                        // 「非好友的单聊(chatType=1)」也升级为 'special'，让前端能浮出并选中导出。
                        // 真正的好友(isFriend)仍标 'friend'，群仍标 'group'，不受影响。
                        const rawClass = isFriend
                            ? 'friend'
                            : isGroup
                            ? 'group'
                            : chatType === 1
                ...
                // Issue #364: 机器人 / 服务号等 special 会话经常没有 peerName / sendNickName，
                // name 只能兜底成裸 peerUid（一长串 u_xxx），前端列表看着像乱码。这里对一部分
                // 仍未命名的 special 项并发查一次 UserApi.getUserDetailInfo 补昵称，整体有上限 +
                // 单条超时 + 全容错，避免拖垮接口。导出阶段 resolveSessionName 本就会再查一次。
                if (includeAll) {
                    const UNNAMED_LIMIT = 30;
                    const unnamed = contacts
                        .filter(
                            c =>
                                c.classification === 'special' &&
                                (!c.name || c.name === c.peerUid) &&
                                typeof c.peerUid === 'string' &&
                                /^u_/.test(c.peerUid),
                        )
                        .slice(0, UNNAMED_LIMIT);
                    await Promise.all(
                        unnamed.map(async (c) => {
                            try {
                                const detail = await Promise.race([
                                    Promise.resolve(userApi.getUserDetailInfo(c.peerUid, false)),
                                    new Promise((_, rej) =>
